...
- **`examples/embers`** — a fourth reference animation, and the first to **compose with the
  ecosystem** rather than reimplement everything in stdlib maths. Warm sparks of this repo's
  own are shed from a background galaxy generated by the **[fresco](https://github.com/ZviBaratz/fresco)**
  provider (`Render(w, h, tick, Galaxy)`), which is dimmed and its dust deepened into a recessed
  wash, with a focal vignette tying the two layers. It answers a fair criticism — that the plugin
  *documented* combining tools (`references/tools.md`) far more than it *demonstrated* it.

  Its one idea past a generic "field + particles" is **coupling that turns a stack into a
  scene**: the sparks *belong to* the galaxy rather than drifting past it. A spark exists only
  where the field is bright beneath it (`fieldGate`), so the bright arms shed sparks and the dark
  void sheds none — and because the arms rotate, the sparks inherit that motion instead of
  fighting it. They stream radially outward from the field's own luminance-weighted **centroid**
  (the real core) and run gold-white at the hot core to deep orange far out — the galaxy's
  warm-core→cool-rim gradient retold in the foreground. It also shows the general shape of
  building on **any** provider whose public surface is rendered output — **parse the ANSI back to
  a cell grid, composite, re-emit** (`parseField`) — and stays a pure, deterministic `Frame`
  (fresco's `Profile` pinned to `TrueColor`; the spark layer hash-driven), free-running like
  `plasma`. Two tests carry *teeth* on the composition itself: `TestFieldLayerPresent` (the
  provider really lit the field) and `TestSparksTrackField` — the coupling test, which asserts
  every visible spark sits over a lit field cell and **zero** over the dark void, so deleting the
  gate goes red. It is the first example module with a dependency, so it ships a `go.sum`.
