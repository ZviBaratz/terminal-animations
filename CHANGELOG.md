...
- **`examples/embers`** — a fourth reference animation, and the first to **compose with the
  ecosystem** rather than reimplement everything in stdlib maths. Warm embers of this repo's
  own drift over a background galaxy generated by the **[fresco](https://github.com/ZviBaratz/fresco)**
  provider (`Render(w, h, tick, Galaxy)`), dimmed into a recessed wash, with a focal vignette
  tying the two layers. It answers a fair criticism — that the plugin *documented* combining
  tools (`references/tools.md`) far more than it *demonstrated* it — with a worked artifact.

  Its one idea past a generic "field + particles" is **coupling**: an ember doesn't sit on the
  galaxy, it *catches its light* — brightness lifted by the field luminance beneath (`glowCouple`)
  and tinted toward the arm it crosses (`hueCouple`), so the layers are driven by each other.
  It also shows the general shape of building on **any** provider whose public surface is
  rendered output — **parse the ANSI back to a cell grid, composite, re-emit** (`parseField`) —
  and stays a pure, deterministic `Frame` (fresco's `Profile` pinned to `TrueColor`; the ember
  layer hash-driven), free-running like `plasma`. Two tests carry *teeth* on the composition
  itself: `TestFieldLayerPresent` (the provider really lit the field) and `TestEmberLayerSparse`
  (the foreground is really there and stays sparse) each go red if their layer is removed. It is
  the first example module with a dependency, so it ships a `go.sum`.
