// cmd/preview — the standalone preview + frame-renderer for the bust animation. This is
// scripts/preview/ (main.go.tmpl + size_*.go) with render() wired to bust.Frame: a live
...
//	go run ./cmd/preview frames 72 1           # dump one whole 72-frame loop (ticks 0..71) — shows motion
//	go run ./cmd/preview frames 72 1 140 70    # …at the native 140×70 pane (the full baked frame)
package main
...
func main() {
	// bust is a pure, free-running loop: frame N is a function of (w, h, N) alone.
	...
	// a slow loop still shows motion, and the size is overridable.
	//   frames N [stride] [w h]
