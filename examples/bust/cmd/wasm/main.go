// cmd/wasm — the browser harness entrypoint for the bust animation.
//
// This is scripts/harness/main.go.tmpl with render() wired to bust.Frame.
// Build and serve it with:
//
//	scripts/harness.sh examples/bust      # → http://localhost:8731/?anim=bust
//
// bust loops every 720 ticks, so setting the harness's compare Δ to 720
...
import (
	...
	"github.com/ZviBaratz/terminal-animations/examples/bust"
)

func main() {
	// bust is a pure, seamless-looping still: frame N is a function of (w, h, N),
	// all motion is colour (the geometry is frozen), and it never resolves.
	render := func(w, h, tick int) (frame string, done bool) {
		return bust.Frame(w, h, tick), false
	}

	// A fixed loop, independent of the pane. Hardcoded rather than exported, since
	// the loop length is a harness concern and not part of what bust offers a
	// caller — but that leaves this literal a hand-kept copy of the unexported
	// `period` constant in bust.go. TestLoopSeam pins that constant, not this
	// copy, so nothing catches the two drifting apart. Change one, change both.
	period := func(w, h int) int { return 720 }
