// cmd/wasm — the browser harness entrypoint for the life animation.
//
// This is scripts/harness/main.go.tmpl with render() wired to a life.Life. Build and
// serve it with:
//
//	scripts/harness.sh examples/life      # → http://localhost:8731/?anim=life
//
// life is stateful and ambient — it advances a Game-of-Life board and reseeds on
// stagnation, so it never exactly repeats. animPeriod therefore returns 0 (the viewer
// disables the compare Δ). Note the harness can only replay forward: scrubbing the
// tick slider backwards drags a stateful animation out of sync until you reload.
//
//go:build js && wasm
...
func main() {
	// Construct once and drive with the absolute tick; View re-seeds on a pane resize.
	// Never resolves, so done is always false.
	l := life.New(80, 24)
	...
	// life is ambient and reseeds on stagnation — it never closes a loop, so there is
	// no compare Δ to offer. 0 means free-running; the viewer says so.
